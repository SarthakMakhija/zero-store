...
// Imagine a Scan operation with a few sorted segments: one active sorted segment and other sorted segments on object store.
// To scan over all these sorted segments, we can create multiple iterators, one for each sorted segment.
// However, we need a composite iterator which merges all these iterators. This is why MergeIterator comes in.
// All these segments are individually sorted by keys, so MergeIterator can use a binary-heap to pick the iterator
// to return the keys in increasing order.
//
// Let's consider an example with 2 iterators, over two sorted segments:
